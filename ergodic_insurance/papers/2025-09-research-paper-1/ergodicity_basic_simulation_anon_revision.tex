...
\begin{itemize}
    ...
    \item Deductible levels vary across [\$0, \$50K, \$100K, \$250K, \$500K, and no insurance]
...
\While{$t < T$ \textbf{and} Equity$_t > 0$}
    ...
    \State $\text{TotalRetained}_t \gets 0$
    \For{each loss $\ell \in \{L_t^{(i)}\}$} \Comment{Per-occurrence insurance}
        ...
        \State Create claim liability for $r(\ell)$ with 10-year payment schedule
        \State Post collateral: Restricted$_t \gets$ Restricted$_t + r(\ell)$
        \State $\text{TotalRetained}_t \gets \text{TotalRetained}_t + r(\ell)$
    \EndFor
    \State Scale premiums: $\text{Premiums}_t \gets \text{Premiums}_0 \times R_t / R_0$
    \State Calculate EBIT: $\text{EBIT}_t \gets R_t \times m - \text{Premiums}_t - \text{TotalRetained}_t$
    ...
    \State Calculate net income: $\text{NI}_t \gets (\text{EBIT}_t - \text{LoC}_t) \times (1 - \tau)$
...
Loss events are processed individually through the insurance structure (per-occurrence basis), so each event incurs its own deductible. The retained loss amount is recognized as an expense in the period of incurrence (accrual basis), reducing EBIT, while the corresponding claim liability is added to the balance sheet and paid over a 10-year schedule (Section~\ref{alg:claim-payments}). The operational volatility shock $\xi_t$ is a multiplicative lognormal factor described in Section~\ref{sec:op-vol}.
...
\State Calculate operating income including all period costs:
\State $\text{EBIT} \gets R \times m - \text{Premiums} - \text{RetainedLosses} - \text{Depreciation}$
\State where RetainedLosses is the sum of company-retained portions of all losses incurred this period
...
\State Taxable income: $\text{TI} \gets \text{EBIT} - \text{LoC\_cost}$
...
Retained losses are recognized as operating expenses when incurred (accrual basis), consistent with the journal entry that simultaneously creates the corresponding claim liability on the balance sheet. Scheduled claim payments in Step~3 affect only the balance sheet: reducing both the claim liability and restricted assets (collateral) without further income statement impact.
...
\begin{equation}
\text{EBIT} = R \times m - \text{Premiums} - \text{RetainedLosses} - \text{Depreciation}
\end{equation}
where $m$ is the base operating margin. Insurance premiums and company-retained losses are both treated as operating expenses. Depreciation is zero in this experiment (PP\&E ratio $= 0$).
...
\end{equation}
where $\tau$ is the corporate tax rate and $\rho$ is the earnings retention ratio. Because retained losses are already included in EBIT, the only additional below-the-line deduction is the letter of credit financing cost. Growth is funded entirely through retained earnings (no external capital).
...
\begin{enumerate}
    ...
    \item Recognizes the retained loss as an operating expense, reducing EBIT for the period
    \item Creates a corresponding claim liability on the balance sheet: Liabilities$_t \gets$ Liabilities$_{t-1} + L_{\text{company}}$
...
Under accrual accounting, the loss expense and the claim liability are two sides of the same journal entry: the expense reduces equity through the income statement while the liability increases the obligations side of the balance sheet, maintaining the accounting equation. \textbf{No cash leaves the company} at claim incurrence. Instead, assets are reclassified from unrestricted to restricted and begin accruing Letter of Credit charges at rate $r_{\text{LoC}}$ (here, 1.5\% annually):
\begin{equation}
...
Equity & Decreases (loss in EBIT) & Affected by LoC costs \\
\bottomrule
\multicolumn{3}{l}{\small *Asset composition changes (Cash $\to$ Restricted) but total unchanged at incurrence.}\\
\multicolumn{3}{l}{\small Equity decreases through the income statement when period NI flows to retained earnings.}
\end{tabular}
\end{center}
...
\textbf{Expense Recognition:} Premiums are recorded as operating expenses in the year incurred, directly reducing EBIT alongside retained losses and depreciation:
\begin{equation}
\text{EBIT}_t = \text{Revenue}_t \times \text{Margin} - \text{Premium}_t - \text{RetainedLosses}_t - \text{Depreciation}_t
\end{equation}
