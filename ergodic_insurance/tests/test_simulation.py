...
class TestSimulationResults:
    ...
    def test_to_dataframe(self):
        ...
        assert len(df) == 3
        assert list(df.columns) == ['year', 'assets', 'equity', 'roe', 
                                     'revenue', 'net_income', 'claim_count', 'claim_amount']
    ...
    def test_summary_stats(self):
        ...
        assert stats['insolvency_year'] is None
...
class TestSimulation:
    ...
    def test_step_annual(self, manufacturer, claim_generator):
        ...
        # Create test claims
        claims = [
            ClaimEvent(year=0, amount=500_000),
            ClaimEvent(year=0, amount=300_000)
        ]
    ...
    def test_memory_efficiency(self, manufacturer, claim_generator):
        ...
        assert actual_memory < 100 * 1024 * 1024, f"Memory usage {actual_memory / 1024 / 1024:.2f} MB exceeds 100MB"
    ...
    def test_insolvency_handling(self, manufacturer_config):
        ...
        sim = Simulation(
            manufacturer=poor_manufacturer,
            claim_generator=harsh_claims,
            time_horizon=100,
            seed=42
        )
