...
time_avg = np.exp(np.mean(np.log(wealth[:, -1] / W0)) / n_steps) ** np.arange(n_steps + 1) * W0
...
def simulate_with_insurance(W0, n_years, premium_rate, retention, n_sims=1000):
    ...
    claim_freq = 3      # 3 claims per year
    ...
    for sim in range(n_sims):
        for year in range(n_years):
            ...
            growth_factor = np.exp((base_growth - 0.5 * volatility**2) + volatility * np.random.randn())

            # Claims
            n_claims = np.random.poisson(claim_freq)
            claims = np.random.lognormal(np.log(claim_severity_mean), 1) * n_claims
