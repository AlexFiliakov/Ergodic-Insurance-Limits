...
from .claim_generator import ClaimGenerator, ClaimEvent
...
class Simulation:
    ...
    def run(self, progress_interval: int = 100) -> SimulationResults:
        ...
        claims_by_year: Dict[int, List[ClaimEvent]] = {year: [] for year in range(self.time_horizon)}
        for claim in all_claims:
            if 0 <= claim.year < self.time_horizon:
                claims_by_year[claim.year].append(claim)
        
        logger.info(f"Starting {self.time_horizon}-year simulation with {len(all_claims)} total claims")
        ...
        for year in range(self.time_horizon):
            ...
            if year > 0 and year % progress_interval == 0:
                ...
                logger.info(f"Year {year}/{self.time_horizon} - {elapsed:.1f}s elapsed, {remaining:.1f}s remaining")
        ...
        results = SimulationResults(
            ...
            claim_counts=self.claim_counts[:year+1] if self.insolvency_year else self.claim_counts,
            claim_amounts=self.claim_amounts[:year+1] if self.insolvency_year else self.claim_amounts,
            insolvency_year=self.insolvency_year
        )
