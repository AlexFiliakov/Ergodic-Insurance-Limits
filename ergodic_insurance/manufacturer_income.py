...
class IncomeCalculationMixin:
    ...
    def calculate_revenue(self, apply_stochastic: bool = False) -> Decimal:
        """Calculate revenue based on operating assets and turnover ratio.

        Revenue is generated by productive (operating) assets only. Deferred
        tax assets represent future tax benefits, not revenue-generating
        capacity, so they are excluded from the asset base (Issue #1055).
        Insurance receivables are claim-related financial assets awaiting
        cash settlement from the insurer — they do not represent productive
        operating capacity and are likewise excluded (Issue #1564).

        Args:
            apply_stochastic (bool): Whether to apply stochastic shock to revenue.

        Returns:
            Decimal: Annual revenue in dollars. Always non-negative.
        """
        ...
        # Exclude insurance receivables — claim-recovery assets do not
        # generate operating revenue (Issue #1564)
        insurance_receivables = getattr(self, "insurance_receivables", to_decimal(0))
        available_assets = max(to_decimal(0), self.total_assets - net_dta - insurance_receivables)
