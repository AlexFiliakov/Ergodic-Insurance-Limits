#!/usr/bin/env python3
"""Fix remaining Sphinx documentation warnings."""

import re

def fix_documentation_warnings():
    """Fix the remaining cross-reference and formatting issues."""
    ...
    # Find and fix the collapsed stochastic-control line (line 1273)
    # This line has everything collapsed together
    pattern = r'\(stochastic-control\)= ## Stochastic Control ### Stochastic Differential Equation.*?```python.*?ax1\.set_ylabel\(\'Wealth \(\$M\)\', fontsize=9\)'

    if '(stochastic-control)= ## Stochastic Control' in content:
        # Extract the collapsed content and reformat it
        replacement = """(stochastic-control)=
...
class StochasticControl:
    ...
    def plot_results(self, V, optimal_control, paths, controls_used, market_conditions):
        ...
        # Find the collapsed line and replace it
        lines = content.split('\n')
        for i, line in enumerate(lines):
            if line.startswith('(stochastic-control)= ## Stochastic Control'):
                # This is the collapsed line
                lines[i] = replacement
                break

        content = '\n'.join(lines)

    # Now fix the convergence-criteria anchor
    # It needs blank lines around it
    content = re.sub(
    ...
    print("Fixed remaining documentation warnings")
