...
class ClaimLiability:
    ...
    year_incurred: int
    payment_schedule: List[float] = field(default_factory=lambda: [
        0.10,  # Year 1: 10%
        0.20,  # Year 2: 20%
        0.20,  # Year 3: 20%
        0.15,  # Year 4: 15%
...
class WidgetManufacturer:
    ...
    def calculate_operating_income(self, revenue: float) -> float:
        ...
        logger.debug(f"Operating income: ${operating_income:,.2f} ({self.operating_margin:.1%} margin)")
        return operating_income
    
    def calculate_collateral_costs(self, letter_of_credit_rate: float = 0.015, 
                                  time_period: str = "annual") -> float:
        ...
        if collateral_costs > 0:
            logger.debug(f"Collateral costs ({time_period}): ${collateral_costs:,.2f} on ${self.collateral:,.2f} collateral")
    ...
    def update_balance_sheet(self, net_income: float, growth_rate: float = 0.0) -> None:
        ...
        logger.debug(f"Balance sheet updated: Assets=${self.assets:,.2f}, Equity=${self.equity:,.2f}")
        if dividends > 0:
        ...
        for claim in self.claim_liabilities:
            ...
            if scheduled_payment > 0:
                ...
                if actual_payment > 0:
                    ...
                    logger.debug(f"Reduced collateral and restricted assets by ${actual_payment:,.2f}")
    ...
    def step(self, working_capital_pct: float = 0.2, 
             letter_of_credit_rate: float = 0.015,
             growth_rate: float = 0.0,
             time_resolution: str = "annual") -> Dict[str, float]:
