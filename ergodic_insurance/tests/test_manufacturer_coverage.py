...
class TestProportionalRevaluation:
    """Tests for _record_proportional_revaluation (lines 1222, 1264-1278)."""

    def test_ratio_equals_one_is_noop(self, manufacturer):
        """Line 1222: ratio == ONE returns early (no adjustment needed)."""
        current_total = manufacturer.total_assets
        entry_count_before = len(manufacturer.ledger)
        manufacturer._record_proportional_revaluation(current_total)
        assert len(manufacturer.ledger) == entry_count_before

    def test_proportional_increase(self, manufacturer):
        """Lines 1240-1259: ratio > 1 increases assets proportionally."""
        current_total = manufacturer.total_assets
        target = current_total * to_decimal(1.5)
        manufacturer._record_proportional_revaluation(target)
        # Assets should have been adjusted upward
        new_total = manufacturer.total_assets
        assert new_total > current_total

    def test_proportional_decrease(self, manufacturer):
        """Lines 1250-1259: ratio < 1 decreases assets proportionally."""
        current_total = manufacturer.total_assets
        target = current_total * to_decimal(0.5)
        manufacturer._record_proportional_revaluation(target)
        new_total = manufacturer.total_assets
        assert new_total < current_total

    def test_accum_depreciation_adjustment_increase(self, manufacturer):
        """Lines 1264-1278: Adjust accumulated depreciation with increase."""
        from ergodic_insurance.ledger import TransactionType

        # Set up accumulated depreciation via ledger (simulating depreciation)
        ...
        assert manufacturer.accumulated_depreciation == to_decimal(200_000)
        current_total = manufacturer.total_assets
        target = current_total * to_decimal(1.3)
        manufacturer._record_proportional_revaluation(target)
        # Accumulated depreciation should have been adjusted proportionally
        assert manufacturer.accumulated_depreciation > ZERO

    def test_accum_depreciation_adjustment_decrease(self, manufacturer):
        """Lines 1276-1278: Adjust accumulated depreciation with decrease."""
        from ergodic_insurance.ledger import TransactionType
        ...
        current_total = manufacturer.total_assets
        target = current_total * to_decimal(0.7)
        manufacturer._record_proportional_revaluation(target)
        assert manufacturer.accumulated_depreciation >= ZERO
