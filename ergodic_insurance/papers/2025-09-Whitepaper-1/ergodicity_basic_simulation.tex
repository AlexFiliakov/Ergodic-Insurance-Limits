...
\begin{itemize}
    ...
    \item Loss distribution parameters (frequency and severity for attritional, large, and catastrophic losses)
...
\begin{itemize}
    \item Working capital is set to 0\% of assets to maximize revenue generation
    \item No stochastic revenue or margin fluctuations
    \item Loss events are independent across time and types
    \item No correlation between losses and business performance (other than through revenue)
    \item Taxes are applied at a flat rate of 25\%
    \item PP\&E and depreciation are set to 0 for simplicity
    \item No external capital raises or dividends
    \item Premiums are fixed at inception and do not change over time
    \item No inflation or economic cycles
    \item Dividend ratio is set at a flat 30\% of net income
    \item No insurer default risk
    \item No regulatory capital requirements
    \item No investment income beyond working capital interest
    \item Payment patterns are a fixed 10-year payout with the following distribution:
        \begin{table}[H]
            \centering
        ...
        \end{table}
    \item Letter of Credit charge on retained loss collateral: 1.5\% annual interest rate
\end{itemize}
