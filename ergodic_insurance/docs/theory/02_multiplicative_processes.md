...
- 1)$  Note: Mean > Median > Mode (right-skewed distribution)  ### Connection to Multiplicative Processes
...
mu_claim = 10
# log-mean (corresponds to ~$22k median)
sigma_claim = 2
...
![Volatility Drag](figures/volatility_drag.png) *Figure 3: The impact of volatility on growth rates, showing how geometric mean decreases with volatility even when arithmetic mean is constant.*
### Mathematical Definition
...
Consider two scenarios: 1. Steady 10% annual return → 1.1^10 = 2.594x after 10 years 2. Alternating +30% and -10% (average 10%) → (1.3 × 0.9)^5 = 2.373x
The volatile path underperforms despite same average.

### Impact on Insurance Decisions

Insurance reduces volatility drag by: 1. Capping downside losses
...
print(f"
Median final wealth: ${np.median(final_with):,.0f}")
print(f"
...
print("\nWithout Insurance:")
print(f"
Median final wealth: ${np.median(final_without):,.0f}")
print(f"
Bankruptcy rate: {np.mean(np.array(final_without) == 0):.1%}")
print(f"
Growth rate: {np.mean(np.log(np.array(final_without)[np.array(final_without) > 0] / 10_000_000) / 20):.2%}")
...
year_return = np.random.randn()
* normal_vol + normal_return
