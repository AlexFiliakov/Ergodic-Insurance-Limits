...
def create_ergodic_distinction():
    ...
    ax2.axvline(np.mean(final_wealth), color='blue', linestyle='-', linewidth=2,
                label=f'Ensemble Avg: {np.mean(final_wealth):.2f}')
    ax2.axvline(np.median(final_wealth), color='red', linestyle='--', linewidth=2,
...
def create_ensemble_vs_time():
    ...
    ax.axvline(np.mean(growth_no), color='red', linestyle='--',
               label=f'Mean (No Ins): {np.mean(growth_no):.3f}')
    ax.axvline(np.mean(growth_with), color='blue', linestyle='--',
    ...
    plt.suptitle('Ergodic Analysis: Insurance Impact on Wealth Dynamics',
...
def create_wealth_trajectories():
    ...
    for event in events:
        ...
        ax.annotate(f'Loss Event', xy=(event[0], event[1]),
    ...
    # Add mean path
    ax.plot([10.0 * (1.05)**t for t in range(n_years)], 'b-',
    ...
    plt.suptitle('Wealth Dynamics and Insurance Analysis',
                 fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def create_growth_rate_distribution():
    ...
    ax = axes[0, 0]
    ax.hist(growth_rates_no_ins, bins=50, alpha=0.5, color='red',
            label='Without Insurance', density=True)
    ax.hist(growth_rates_with_ins, bins=50, alpha=0.5, color='blue',
            label='With Insurance', density=True)
    ...
    ax = axes[1, 0]
    bp = ax.boxplot([growth_rates_no_ins, growth_rates_with_ins],
    ...
    plt.suptitle('Growth Rate Analysis: Impact of Insurance',
                 fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
